...
public class SwerveDriveOdometry extends CommandBase {
  ...
  public void execute() {
    ...
    if(Math.round(pos.getX()*20) != Math.round(targetXPos*20)){
      if(pos.getX() < targetXPos) {
        x_vel = () -> Constants.AUTONOMOUS_VELOCITY_PER_SECOND * 0.05;
    ...
    if(Math.round(pos.getY()*20) != Math.round(targetYPos*20)){
      if(pos.getY() < targetYPos) {
        y_vel = () -> Constants.AUTONOMOUS_VELOCITY_PER_SECOND * 0.05;
    ...
    if(Math.round(targetTurnDegrees/5) == -Math.round(RobotContainer.m_swerve.gyroAngle().getDegrees()/5))
      ...
      turn_vel = () -> 0.05 * Constants.MAX_RADIANS_PER_SECOND * Math.copySign(1, -targetTurnDegrees + RobotContainer.m_swerve.gyroAngle().getDegrees());



    RobotContainer.m_swerve.setChasisSpeed(ChassisSpeeds.fromFieldRelativeSpeeds(x_vel.getAsDouble(), y_vel.getAsDouble(),
        turn_vel.getAsDouble(), RobotContainer.m_swerve.gyroAngle()));
  ...
  public boolean isFinished() {
    if((Math.round(pos.getX()*20) == Math.round(targetXPos*20)) && ((Math.round(pos.getY()*20) == Math.round(targetYPos*20))) && (Math.round(pos.getRotation().getDegrees()) == (Math.round(targetTurnDegrees)))) {
